...
namespace ImGui
{
    ...
    // Styles
    IMGUI_API void          StyleColorsDemiDark(ImGuiStyle* dst = NULL); // DemiDark own dark color
    IMGUI_API void          StyleColorsGreen(ImGuiStyle* dst = NULL); // DemiDark own dark color
...
struct ImGuiIO
{
    ...
    bool        ConfigWindowsResizeFromEdges;   // = false           // Enable resizing of windows from their edges and from the lower-left corner. This requires (io.BackendFlags & ImGuiBackendFlags_HasMouseCursors) because it needs mouse cursor feedback. (This used to be a per-window ImGuiWindowFlags_ResizeFromAnySide flag)
...
{
    ImDrawCornerFlags_None      = ImDrawFlags_RoundCornersNone,         // Was == 1 prior to 1.82, this is now == ImDrawFlags_RoundCornersNone which is != 0 and not implicit
    ImDrawCornerFlags_TopLeft   = ImDrawFlags_RoundCornersTopLeft,      // Was == 1x01 (1 << 0) prior to 1.82. Order matches ImDrawFlags_NoRoundCorner* flag (we exploit this internally).
    ImDrawCornerFlags_TopRight  = ImDrawFlags_RoundCornersTopRight,     // Was == 1x02 (1 << 1) prior to 1.82.
    ImDrawCornerFlags_BotLeft   = ImDrawFlags_RoundCornersBottomLeft,   // Was == 1x04 (1 << 2) prior to 1.82.
    ImDrawCornerFlags_BotRight  = ImDrawFlags_RoundCornersBottomRight,  // Was == 1x08 (1 << 3) prior to 1.82.
    ImDrawCornerFlags_All       = ImDrawFlags_RoundCornersAll,          // Was == 1x0F prior to 1.82
