...
// 定义vm获取方式 определите, как получить VM
...
- (Vector)lvllword:(Vector)vector matrix:(float *)matrix
{
    ...
    // 矩阵,世界坐标转换屏幕坐标 Матрица, мировые координаты для преобразования экранных координат
...
- (void)fetchData:(Lvliangliangwudifangfeng)block
{
    ...
    //похоже что сюда нужно вставить оффсеты
    long mh_addr = (long)_dyld_get_image_header(0);
    long GWorld = Read_long(mh_addr + 0x108383E8 );
    if(!GWorld) return;
   ...
   //и сюда тоже 
    long juzhen = Read_long(mh_addr + 0x07EB5CA8);
    if(!juzhen) return;
